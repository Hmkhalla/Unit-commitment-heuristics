...
set T;	#set of time periods in the considered time horizon
set J;	#set of generation units

param c{J} > 0;	#cost for generating electricity by unit j
param c_u{J} > 0;	#startup cost of unit j
param D{T};	#electricity demand to be satisfied at time period t
param R{T}; #amount of spinning reserves at time period t
...
var p{j in J, t in T};	#electricity produced by unit j at time period t
var p_max{j in J, t in T};	#maximum electricity generated at each time period by unit j
...
subject to status_update{j in J, t in T}:
	if t==1 then v[j,t] - v0[j] - (y[j,t] - z[j,t])
          else v[j,t] - v[j,t-1] - (y[j,t] - z[j,t])
	= 0;
...
subject to p_max_ramping_down{j in J, t in T}:
